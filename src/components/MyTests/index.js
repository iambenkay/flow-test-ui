    ...
    renderTests() {
        return this.props.tests.map(test => (
            <ListGroupItem key={test.id} className="text-left">
                {test.name}
            <button className="btn ml-3 btn-success" onClick={() => { this.props.onClick(test.id)}}>Manage Test</button>
            </ListGroupItem>
        ))
    }
...
export default withRouter(withSession(withAuthorization(MyTests)));
